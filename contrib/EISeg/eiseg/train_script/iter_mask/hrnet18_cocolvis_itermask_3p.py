...
MODEL_NAME = 'cocolvis_hrnet18_ocr64_iter3p'
...
def init_model(cfg):
    ...
    model = HRNetModel(width=18, ocr_width=64, small=False, with_aux_output=True, use_leaky_relu=True,
...
def train(model, cfg, model_cfg):
    ...
    train_augmentator = Compose([
        ...
        RGBShift(r_shift_limit=10, g_shift_limit=10, b_shift_limit=10, always_apply=False, p=0.75)
    ], p=1.0)
    ...
    trainer = ISTrainer(model, cfg, model_cfg, loss_cfg,
                        ...
                        with_prev_mask =True,
                        prev_mask_drop_prob=0.2)
    trainer.run(num_epochs=56)