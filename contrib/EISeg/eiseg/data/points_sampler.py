...
class BasePointSampler:
    ...
    def selected_mask(self, mask):
        self._selected_mask = mask[np.newaxis, :].astype('float32')
...
class MultiPointSampler(BasePointSampler):
    ...
    def sample_object(self, sample: DSample):
        if len(sample) == 0:
            ...
            self.selected_mask = np.zeros_like(bg_mask, dtype='float32')
    ...
    def _positive_erode(self, mask):
        ...
        eroded_mask = cv2.erode(mask.astype(np.uint8),kernel, iterations=self.positive_erode_iters).astype('bool')
